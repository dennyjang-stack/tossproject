...
브라우저 검증은 별도 상태에서 `/` 직접 접근 → `/login` 이동 → 공백·형식·자격 오류 표시 →
시드 계정 로그인 → `/`와 사용자 이름 확인 → 로그아웃 → `/login` → 다시 `/` 접근 시 차단의
단일 여정으로 한다. 마지막으로 소스 검색을 통해 `http://localhost:8080`이 rewrite 외부에 없고,
외부 `http(s)` 자산과 `be/` 변경이 없는지 확인한다.
