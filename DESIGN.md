...
사이클 2에서는 기능 범위를 넓히지 않고, verify에서 드러난 브라우저 검증 환경의 오염만
...
| 전체 브라우저 흐름 | 스텁 쿠키 기반 로그인→홈→로그아웃 | :3000에서 agent-browser로 실제 입력·클릭 |
...
로그아웃 뒤 `/api/me` 401 순서로 수행한다. 각 오류에서 `timestamp`, `status`, `message`,
`errors` 네 키와 ISO-8601 UTC timestamp를 확인한다.

브라우저 검증은 별도 상태에서, 반드시 새로 시작한 `npm run dev:fresh` 인스턴스를 대상으로
`/` 직접 접근 → `/login` 이동 → 공백·형식·자격 오류 표시 → 시드 계정 로그인 → `/`와 사용자
이름 확인 → 로그아웃 → `/login` → 다시 `/` 접근 시 차단의 단일 여정으로 한다. 이전
...
UTC 오류 형식을 순서대로 점검한다. 마지막으로 소스 검색을 통해 `http://localhost:8080`이
rewrite 외부에 없고, 외부 `http(s)` 자산과 `be/` 변경이 없는지 확인한다.
