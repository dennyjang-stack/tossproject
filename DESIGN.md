...
이번 세대의 TRD는 교차검증 정합성(전 항목 pass) 뒤 재생성되며 조건 문구가 더 정밀해졌다.
설계상 대응해야 할 강화점은 세 가지다.
...
2. **각 조건의 검증 방법이 명시**됐다(기대 상태코드·경계 입력·기대 DOM/URL). 8절의 추적표를
   이 방법들과 글자 단위로 맞춰 두어, verify가 애매함 없이 재현하도록 한다.
3. **DESIGN.md 관찰 기록에 구체 색값**(`#3182f6`·`#191f28`·`#4e5968`)과 구현 버튼 계산값
   (`rgb(49,130,246)`) 일치가 요구된다. 2절이 이 값을 이미 담고 있고, 구현의 `.primary-button`
   배경이 그 값과 일치함을 verify에서 계산 스타일로 확인한다.

현재 `fe/` 구현은 위 강화 조건을 모두 반영한 상태이며, 이번 사이클의 설계는 기능 확장이 아니라
재생성된 조건과 구현·검증 절차의 정합을 확정하는 데 있다. 새 요구가 생기면 이 문서를 갱신할
기준점으로 남긴다. `npm run dev:fresh`는 먼저 `:3000` 사용 가능 여부를 확인하고, 점유 중이면
...
아래 표의 행 순서와 문구는 재생성된 TRD의 인수 조건 순서와 1:1로 맞춘다.
...
| 계약 스텁 3개 계약 일치 | 공통 `stub-auth`와 세 Route Handler, 고정 응답 문구·쿠키 정책 | `npm run verify:stub` 종료 0: 공백 400(email·password `errors[]`), 잘못된 자격 401, 시드 200 `{name:"토스사용자"}`+`Set-Cookie … HttpOnly`, 무쿠키 401, 로그아웃 204(`Max-Age=0`) |
...
| 성공 `router.replace`·401 인라인 | 로그인 상태 머신의 200/401 분기 | 시드 제출 후 URL 경로 `/`와 `토스사용자` 표시, 틀린 자격 후 `/login` 유지 + 경고 문구 |
...
| 전체 브라우저 흐름 | 스텁 세션 기반 로그인→홈→로그아웃 | 새로 시작한 `npm run dev:fresh` 서버에서 agent-browser 실제 입력·클릭 |
| `be/` 미수정 | 모든 구현을 `fe/`로 제한 | `git diff --name-only`에서 `be/` 부재 확인 |
| strict 빌드·취약점 0 | 엄격한 TypeScript와 최소 의존성 | `cd fe && npm ci`(취약점 0건)와 `npm run build` 각각 종료 코드 0 |
