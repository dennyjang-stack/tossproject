...
정리한다. 구체적으로는 `npm run dev`로 새로 띄운 개발 서버 하나를 검증 기준으로 삼고,
이전 `:3000` 프로세스가 남아 있으면 먼저 종료한 뒤 브라우저 흐름을 다시 확인한다.
...
브라우저 검증은 별도 상태에서, 반드시 새로 시작한 `npm run dev` 인스턴스를 대상으로
...
기록한다. 마지막으로 소스 검색을 통해 `http://localhost:8080`이 rewrite 외부에 없고, 외부
`http(s)` 자산과 `be/` 변경이 없는지 확인한다.
