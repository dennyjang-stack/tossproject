...
사이클 4에서는 기능을 더하지 않고, `npm ci`에서 보고된 감사 경고 2건을 줄이는
의존성 정리만 다룬다. `fe/package.json`과 `fe/package-lock.json`의 버전 조정만 허용하고,
로그인·홈·로그아웃의 런타임 동작과 `/api/*` 계약은 그대로 유지한다. 이 범위는
`npm run build`, `npm run verify:stub`, 새 개발 서버의 브라우저 여정으로 즉시 검증할 수 있다.

사이클 13에서도 추가 기능 구현은 없다. 모든 TRD 인수 조건과 열린 개선 항목은 이미 정리되어
다음 빌드 범위는 비어 있으며, 남은 작업은 현재 상태의 재검증과 브랜치 종료뿐이다. 이 문서는
이후 새 요구가 생기면 갱신할 기준점으로 남긴다.
