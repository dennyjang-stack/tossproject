...
> 사이클 2 · design 페이즈 갱신. 대응 TRD = `TRD.md`(재생성 후 인수 조건 9개, 전부 `- [ ]`).
> **`be/`만 다룬다. `fe/`는 건드리지 않는다.** 프론트 없이 MockMvc 테스트로 검증을 완결한다.

## 0. 사이클 2 상태 요약 (설계 유효성 재확인)

사이클 1에서 재생성 TRD의 강화된 인수 조건 9개를 모두 구현·테스트했고(`AuthControllerTest`
17개, `./gradlew clean build` 종료 코드 0), VERIFY가 9개 전부에 대한 green 근거를
`## Verify`에 기록했다. 다만 verify가 TRD.md 체크박스를 토글하지 않아 9개가 아직 `- [ ]`로
남아 있다(그래서 새 사이클이 돈다). **이번 사이클에서 설계·구조 변경은 없다** — 아래 설계는
이미 구현·검증된 것과 정확히 일치하며, implement는 현 산출물이 이 설계·TRD와 어긋나지 않는지
점검(정규식 동일성·경계 테스트·응답 필드 개수)하고, verify가 코드 변경 없이 9개 박스를
토글하면 루프가 수렴한다. 설계를 그대로 유지하는 판단의 근거를 §0에 명시하고, 세부 설계는
아래 §1~§10을 그대로 승계한다.

## 1. 이번 사이클 범위

TRD의 인수 조건 전체가 "세션 로그인 API 3종 + 공통 에러 처리"라는 하나의 응집된 묶음이다.
현재 구현은 이 묶음을 이미 완성했으므로 구조를 다시 만들지 않는다. 이번 사이클의 설계 목표는
**재생성된 (trd-regen) TRD**의 강화된 인수 조건과 현 산출물이 계속 일치함을 고정하는 것이다.
재생성에서 강화된 핵심은 다음과 같으며, 설계는 이를 이미 반영하고 있다.
...
현재 구현·테스트는 사이클 1에서 재생성 TRD의 인수 조건 9개를 이미 충족했다(테스트 17개, 빌드 0).
따라서 이번 사이클 implement는 **새 구조 없이 점검·고정만** 수행한다 — 아래 항목이 이미 만족돼
있는지 확인하고, 어긋난 부분만 최소 수정한다. 전부 만족돼 있으면 코드 변경 없이 빌드 green을
재확인하는 것으로 충분하다(그 뒤 verify가 TRD 체크박스 9개를 토글해 수렴).
