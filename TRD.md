...
- [x] `agent-browser`로 `https://toss.im`을 열어 스냅샷/스크린샷으로 스타일 재현 포인트를 확인하고 DESIGN.md에 기록했다
- [x] 계약 스텁 3개(`/api/login`·`/api/me`·`/api/logout`)가 계약 표와 동일한 상태코드·필드명·에러 형태·시드 계정으로 동작한다
- [x] `/login`이 이메일·비밀번호 입력과 로그인 버튼을 갖춘 토스 스타일(미니멀 카드, 파란 프라이머리 버튼) 폼을 렌더한다
- [x] 로그인 성공 시 `router.replace`로 `/`에 리다이렉트되고, 실패(401) 시 폼에 인라인 에러 메시지를 보여준다
- [x] 잘못된 이메일 형식·공백 제출 시 400 응답의 `errors[]`를 필드별 인라인 메시지로 보여준다
- [x] 홈(`/`)이 로그인 사용자 이름(`토스사용자`)을 표시하고, 로그아웃 클릭 시 `/login`으로 돌아간다
- [x] 미로그인 상태로 `/` 접근 시 `/login`으로 리다이렉트된다
- [x] 뷰포트 360px~1280px에서 레이아웃이 깨지지 않고, 외부 CDN·폰트·이미지 링크가 없다
- [x] `NEXT_PUBLIC_API_MODE=real`이면 rewrite(`/api/:path*` → `:8080`)로 전환되는 설정이 존재한다 (코드는 항상 상대 경로 `/api/...`만 호출)
- [x] `agent-browser`로 dev 서버(:3000, 스텁 모드)에서 로그인 → 홈 리다이렉트 → 로그아웃 실동작을 확인했다
- [x] `be/` 폴더를 수정하지 않았다 (`git diff --name-only`에 `be/` 없음)
- [x] `npm run build`가 종료 코드 0으로 끝난다 (TypeScript strict 통과)
