...
- [x] `POST /api/login`에서 잘못된 이메일 형식 또는 이메일·비밀번호 공백을 보내면 각각 **400**을 반환하고, `errors[]`에 해당 입력 필드(`email`/`password`)의 `field`가 정확히 일치하고 `message`가 비어 있지 않은 `{field, message}` 항목이 포함된다. 이메일과 비밀번호가 동시에 잘못된 요청은 `errors[]`에 두 필드 항목이 모두 담긴다
...
- [x] `POST /api/logout`은 세션 유무와 관계없이 본문 없는 **204**를 반환하고(세션이 없어도 204), 기존 세션이 있으면 무효화하여 같은 세션으로 직후 호출한 `GET /api/me`가 **401**이 된다. 세션 있음·없음 두 경우를 별도 테스트로 검증한다
- [x] API 계약상 **400·401** 에러 응답은 모두 `timestamp`, `status`, `message`, `errors` 네 필드를 포함하며, `status`(정수)는 실제 HTTP 상태 코드와 일치하고 `errors`는 항상 배열(JSON array)이다
...
- [x] 위 요청 행렬의 성공·실패(400/401)·경계 케이스와 HttpOnly 세션 쿠키 설정을 한국어 `@DisplayName`의 `@WebMvcTest` + `MockMvc` 테스트로 각 인수 조건마다 성공·실패 쌍이 존재하도록 검증하며, `be/`에서 새로 실행한 `./gradlew clean build`가 테스트 실패 없이 `BUILD SUCCESSFUL`·종료 코드 0으로 끝난다
